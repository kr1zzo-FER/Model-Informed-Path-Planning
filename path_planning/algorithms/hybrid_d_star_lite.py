...
class DStarLite:
    ...
    def detect_zone(self, coordinate):
        # "c" value in zones
        try:
            if self.zones[coordinate] == "c":
                return math.inf
            elif self.zones[coordinate] == "r":
                return self.red_cost
            elif self.zones[coordinate] == "y":
                return self.yellow_cost
            elif self.zones[coordinate] == "g":
                return self.green_cost 
            else:
                return 1
        except KeyError:
            return 1
    ...
    def c(self, node1: Node, node2: Node):
        factor = self.detect_zone((node2.x, node2.y))
