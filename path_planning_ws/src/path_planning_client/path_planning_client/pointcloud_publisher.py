...
class CoastToPointCloud(Node):
    ...
    def __init__(self):
        ...
        self.safe_zone_pcd_points = sensor_msgs.PointCloud2()
        self.path_pcd = sensor_msgs.PointCloud2()
        ...
        # safety zone points
        self.safe_zone_pcd_publisher = self.create_publisher(sensor_msgs.PointCloud2, 'safe_zone_pcd_points', 10)
        #path points
    ...
    def gps_coordinates_callback(self, msg):
        ...
        if self.first_gps_msg:
            ...
            self.safe_points = [(msg.safe_points_x.data[i], msg.safe_points_y.data[i]) for i in range(len(msg.safe_points_x.data))]
            ...
            for point in self.safe_points:
                self.zones_dictionary[tuple(point)] = "s"

            self.lcc = LocalCoordinatesConverter(self.zones_dictionary, self.grid_size)

            coast_points_m, red_points_m, yellow_points_m, green_points_m, safe_points_m = self.lcc.get_points_m()
            ...
            self.safe_zone_pcd_points = self.convert_to_pcd(safe_points_m, 0)
    ...
    def timer_callback(self):
        ...
        self.safe_zone_pcd_publisher.publish(self.safe_zone_pcd_points)
